...
bp.list = [
	new bp.create('<b>Factory</b>',		10000000,		51840,		'parts',	1, 1.07, 48),
	new bp.create('<b>Music Studio</b>',	500000000,	622080,		'records',	1, 1.07, 96),
	new bp.create('<b>Bank</b>',			1200000000,	7464961,	'loans',		1, 1.07, 192),
	new bp.create('<b>Tech Company</b>',			2000000000000,	89579521,	'computers',		1, 1.07, 576)
];
...
bb.update = function() {
	for (var i = 0; i < bb.list.length; i++) {
		...
		$("#bb-infos-" + (i+1)).html("Owned : " + bb.owned[i] + "<br>+" + "$" + fix(h.bb.getMoneyReward(i), 2) + "<br>+" + "$" + fix(h.bb.getMps(i), 2) + "/sec<br>Cost " + "$" + fix(b, 2));
	};
};
bp.update = function() {
	for (var i = 0; i < bp.list.length; i++) {
		...
		$("#bp-infos-" + (i+1)).html("Owned : " + bp.owned[i] + "<br>+" + "$" + fix(h.bp.getMoneyReward(i), 2) + "<br>+" + "$" + fix(h.bp.getMps(i), 2) + "/sec<br>+" + fix(h.bp.getRessourceReward(i), 2) + " " + a.ressource + "<br>Cost " + "$" + fix(b, 2));
	};
};
// buying functions
bb.buy = function(i) {
